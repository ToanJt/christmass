...
export const Experience: React.FC<ExperienceProps> = ({ mode, handPosition, uploadedPhotos, twoHandsDetected, onClosestPhotoChange, isMobile = false }) => {
  const controlsRef = useRef<any>(null);
  const foliageCount = isMobile ? 6000 : 12000;
  const ornamentCount = isMobile ? 250 : 600;
  ...
  return (
    <>
      ...
      <spotLight 
        ...
        angle={0.2} 
        penumbra={1} 
        intensity={isMobile ? 1.4 : 2} 
        color="#fff5cc" 
        castShadow 
      />
      <pointLight position={[-10, 5, -10]} intensity={isMobile ? 0.6 : 1} color="#D4AF37" />

      <group position={[0, -5, 0]}>
        <Foliage mode={mode} count={foliageCount} />
        <Ornaments mode={mode} count={ornamentCount} />
